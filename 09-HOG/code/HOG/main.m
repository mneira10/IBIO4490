...
% X is a D by N matrix, with one column per example and D feature dimensions (SINGLE or DOUBLE).
% Y is a DOUBLE vector with N elements with a binary (-1 or +1) label for each training point.

num_features_pos = size(features_pos,1) ;
num_features_neg = size(features_neg,1) ;
X = [features_pos', features_neg'] ;
Y = [ones(1,num_features_pos), -1*ones(1,num_features_neg)] ;
